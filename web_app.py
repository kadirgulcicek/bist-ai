...
<div class="card"><h2>Halka Arz Takibi</h2><p class="muted">HalkArz.com verileriyle son 6 halka arz. Piyasa başlangıcından itibaren 14 gün takip edilir.</p><p><b>{{ veri_sayisi }}</b> kayıt | <b>{{ fiyatli_veri_sayisi }}</b> son fiyat mevcut | Kaynak: <b>{{ veri_kaynagi }}</b></p><a href="/halka-arz?yenile=1" style="display:inline-block;background:#e94560;color:white;text-decoration:none;padding:9px 14px;border-radius:5px;font-weight:bold">Veriyi şimdi yenile</a><p class="muted">Son kontrol: {{ son_guncelleme }}</p></div>
